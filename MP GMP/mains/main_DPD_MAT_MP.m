...
%% Model User inputs
miu_MP    = -1;
mem_depth = 10 ;                                           %M in the MP model
...
z_hat         = [zeros(mem_depth,1);PD_MP(y./G, coef, mem_deg, mem_depth)];
error         = z - z_hat;
...
z = [zeros(mem_depth,1);PD_MP(x_new, coef, mem_deg, mem_depth)];
%[y_MP, ~, ~, ~] = RFWebLab_PA_meas_v1_1(z, RMS_in);
 y_MP  = [zeros(mem_depth,1); Get_model_output_MP(AMP_coef_Matrix, z, mem_deg, mem_depth)];
 y_MP  = y_MP.*G;
%[y_no_MP, RMSout, Idc, Vdc] = RFWebLab_PA_meas_v1_1(x_new.*G, RMS_in);
 y_no_MP  = [zeros(mem_depth,1); Get_model_output_MP(AMP_coef_Matrix, x_new, mem_deg, mem_depth)];
