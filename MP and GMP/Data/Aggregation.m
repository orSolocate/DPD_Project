% To Change Data: Ctrl+F "DATA IN HERE"
% To Get Aggregated Waveform: Ctrl+F "TAKE WAVEFORM HERE"
...
% This example shows how multiple downlink carriers can be generated,
% aggregated and further demodulated using the LTE System Toolbox(TM).

% Copyright 2011-2016 The MathWorks, Inc.

%% Introduction
% This example models an LTE Release 12 waveform with Carrier Aggregation
% (CA). The number of subcarriers and their bandwidth can be specified as a
% parameter. An intra-band contiguous CA case is considered.
%
% To generate an aggregated downlink waveform an eNodeB is configured for
% each component carrier. The component carrier parameters are calculated
% and used to generate a modulated waveform for each eNodeB configuration.
% All the CC modulated waveforms are resampled to a common sampling rate so
% they can be combined to create an aggregated waveform.

%% Number of Component Carriers and Bandwidths
% A vector |NDLRB| specifies the number of resource blocks (RBs) for each
% component carrier (CC). The length of this vector corresponds to the
% number of CCs. The elements of |NDLRB| must be in the set {6, 15, 25, 50,
% 75, 100} RBs. TS 36.101 Table 5.6A.1-1 [ <#16 1> ] lists the valid
% combinations of bandwidths for carrier aggregation.

% Configure the set of NDLRB values to describe the carriers to be
% aggregated
NDLRB = [100 25 100 25 100];
...
tx = cell(1,numCC);
for i = 1:numCC
    tx{i} = lteRMCDLTool(enb{i},randi([0 1],1000,1)); %DATA IN HERE
...
% waveform=tx{1};
% waveform = waveform + tx{2}+tx{4}+tx{5};
...
specPlot = hCarrierAggregationPlotSpectrum(waveform,SR,...
    'Power Spectrum of Carrier Aggregation',{'Signal spectrum'}); %TAKE WAVEFORM HERE
